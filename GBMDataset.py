...
class GBMDataset(MatDataset):
    ...
    def configure_dataset_spinn(self, Nt):
        self.batch = {}
        
        # Prepare residual batch
        # tgrid for residual
        t_res = torch.linspace(0, 1, Nt, device=self.device).reshape(-1, 1)
        
        # spatial coordinates
        coords = []
        if 'gx_scaled' in self:
            coords.append(self['gx_scaled'].clone().detach().to(self.device).reshape(-1, 1))
        if 'gy_scaled' in self:
            coords.append(self['gy_scaled'].clone().detach().to(self.device).reshape(-1, 1))
        if 'gz_scaled' in self:
            coords.append(self['gz_scaled'].clone().detach().to(self.device).reshape(-1, 1))
        ...
        # Prepare data batch
        # tgrid for data
        t_dat = self['t_scaled'].clone().detach().to(self.device).reshape(-1, 1)
        ...
        # Spatial fields
        for key in self.SPATIAL_FIELDS:
            if key in self:
                val = self[key].clone().detach().to(self.device)
                self.batch['dat'][key] = val
                self.batch['res'][key] = val

        # Spacetime fields
        for key in ['uchar', 'ugt']:
            if key in self:
                val = self[key].clone().detach().to(self.device)
                # val is (nx, ny, nz, nt)
                # permute to (nt, nx, ny, nz)
                if xdim == 1:
                    # (nx, nt) -> (nt, nx)
                    val = val.permute(1, 0)
                elif xdim == 2:
                    ...
                    # (nx, ny, nz, nt) -> (nt, nx, ny, nz)
                    val = val.permute(3, 0, 1, 2)
    ...
    def visualize_sampling(self, savedir=None):
        ...
        if savedir is None:
            return
        if 'res' not in self.iter or 'dat' not in self.iter:
            print("Cannot visualize sampling: dat/res iterators not configured.")
            return
    ...
    def restore_to_full_grid(self, full_shape, fields, coords=None):
        """
        Maps cropped fields back to a full grid of zeros.
        
        Args:
            full_shape (tuple): Shape of the full grid (Nx, Ny, Nz) or (Nx, Ny).
            fields (ndarray ): Field to restore. 
            coords (tuple/list of ndarray, optional): (gx, gy, gz) 1-based coordinates.
                                                      If None, uses self['gx'], self['gy'], etc.
        ...
        # Get coordinates
        if coords is None:
            coords_keys = ['gx', 'gy', 'gz'][:xdim]
            coords_list = [self[k] for k in coords_keys]
        else:
            coords_list = coords

        # Get 0-based indices for each dimension
        indices = []
        for c in coords_list:
            # Ensure 1D array and convert 1-based to 0-based
            c_arr = np.asarray(c).reshape(-1)
            indices.append((c_arr - 1).astype(int))
            
        # Create meshgrid for indexing the full array
        ix = np.ix_(*indices)
        
        def _restore_one(data):
            # Construct full shape preserving trailing dimensions
            out_shape = full_shape + data.shape[xdim:]
            full_grid = np.zeros(out_shape, dtype=data.dtype)
            full_grid[ix] = data
            return full_grid

        return _restore_one(fields)
...
class GBMDataIterableDataset(IterableDataset):
    ...
    def __iter__(self):
        ...
        while True:
            ...
            if ugt is not None:
                out["ugt_dat_train"] = ugt[idx]
            if uchar is not None:
                out["uchar_dat_train"] = uchar[idx]
